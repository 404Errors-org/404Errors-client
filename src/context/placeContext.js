...
export const PlaceProvider = ({ children }) => {
  ...
  const changeSelectLocation = (newLocation) => {
    if (lastSelectedLocationRef.current && lastSelectedLocationRef.current.id === newLocation.id) {
  ...
  return (
    <PlaceContext.Provider
      value={{ selectedLocation, infoVisible, changeSelectLocation, toggleInfoVisible, setModal, modal }}
    >
      {children}
    </PlaceContext.Provider>
  );
